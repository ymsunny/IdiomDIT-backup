...
# -*- coding: utf-8 -*-
"""
clean_fi_en.py — 清洗 Fi→En 习语数据集

原始数据：nightingal3/idiom-translation
  metaphor-translation/data/test_sets_final/fi/idiomatic_all.csv

输出 schema 与 En_Fa.csv / Fa-En.csv 一致：
  idiom, sentence, gold translation

用法：
  python scripts/clean_fi_en.py --input <path_or_url> --output Data/Fi-En-Idiom/ParallelData/Fi_En_clean.csv
"""
...
DEFAULT_OUTPUT = os.path.join(
    ...
    "Data", "Fi-En-Idiom", "ParallelData", "Fi_En_clean.csv"
)
...
def clean(rows: list[dict]) -> tuple[list[dict], dict]:
    ...
    for row in rows:
        idiom = row.get("contains_idioms", "").strip()
        sentence = row.get("original_text", "").strip()
        gold = row.get("text", "").strip()

        # 去掉复合词标注符号 #（如 poikki#telo → poikkitelo）
        idiom = idiom.replace("#", "")
...
def verify(rows: list[dict]) -> None:
    ...
    for i, row in enumerate(rows):
        if "," in row["idiom"]:
            print(f"  [WARN] 行 {i+1}: idiom 含逗号: {row['idiom']!r}")
            issues += 1
    if issues == 0:
...
def main():
    parser = argparse.ArgumentParser(description="清洗 Fi→En 习语数据集")
