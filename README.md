...
- `API_BASE_URL` — currently a third-party OpenAI-compatible relay endpoint used by the original experiments (`https://api2.aigcbest.top/v1`), **not** the official OpenAI API. This is a plain constant in `config.py`, not read from `.env` — point it at whatever endpoint serves your judge model.
